...
parser.add_argument('reference', help='path for input fasta',
                    type=str)
parser.add_argument('--minhrun', help='minimum homopolymer run length (0=disabled)',
                    type=int, default=0)
parser.add_argument('--minrepeats', help='minimum dinucleotide repeats (0=disabled)',
                    type=int, default=0)
parser.add_argument('--threads', help='number of threads (1)',
                    type=int, default=1, choices=range(1, cpu_count()))
...
PATTERN = ''
if args.minrepeats:
    PATTERN = ('|').join(["(%s){%s,}" % (x, args.minrepeats)
                          for x in DINUC_REPEATS])
if args.minhrun:
    if PATTERN:  PATTERN += '|'
...
def find_repeats(seq_record, pattern=PATTERN):
    ...
    for repeat in re.finditer(pattern, str(seq_record.seq), flags=re.I):
        ...
        repeat_list.append((seq_record.id,
...
outfile = args.reference + \
          '.dinuc_repeats_ge' + str(args.minrepeats) + \
          '.hrun_ge' + str(args.minhrun) + \
          '.bed'
